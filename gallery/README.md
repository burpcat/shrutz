...
This directory holds the maintainer-facing template for the manifest that `shrutz gallery list`/`shrutz gallery install` actually fetch from the dedicated **`burpcat/shrutz-wallpaper-repo`** GitHub repository (via `raw.githubusercontent.com`) — a small catalog of developer-curated wallpaper sets that any shrutz user can browse and optionally download. It is not bundled with a fresh install; `shrutz` ships with no images by default (see the installer's own prompt), and this is purely an opt-in "browse more sets" feature. Publishing is manual and single-source by design — there is no automated pipeline pushing content from this code repo to the wallpaper repo, and no secondary/fallback content source.
...
2. Upload that zip as a GitHub Release asset in the `burpcat/shrutz-wallpaper-repo` repo (not committed directly — keeps clone size small).
...
```json
{
  ...
  "images": <count>,
  "thumbnail_url": "https://raw.githubusercontent.com/burpcat/shrutz-wallpaper-repo/master/gallery/thumbs/<name>.jpg",
  "download_url": "https://github.com/burpcat/shrutz-wallpaper-repo/releases/download/<release-tag>/<name>.zip"
}
```
