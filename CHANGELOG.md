...
**Menu bar app: fixed the "muddy" look from v2.5.0's rebuild.** The v2.5.0 pass was itself still visually wrong — every surface rendered as a flat, dusty taupe wash with no visible card depth, a dark-puck rotary dial, and a single-bar "z" instead of the approved double-bar "≠"-style strike. Root-caused to two independent, compounding bugs rather than one: (1) the wallpaper color sampler averaged raw RGB per region, which collapses toward grey/brown whenever a region mixes saturated and desaturated pixels (e.g. skin tones against a shadowed backdrop); (2) the five ambient color blobs composited with ordinary alpha blending under `.ultraThinMaterial`, which regresses further toward grey on overlap and desaturates again on top of that. Fixed with an HSB, saturation-weighted color extraction (circular hue averaging, so 350°/10° correctly average near 0° instead of collapsing to 180°) plus a compositing fix (`.compositingGroup()` + `.blendMode(.plusLighter)` + `.saturation(1.3)`) so overlapping colors glow instead of muddying. Verified against two very different wallpaper sets — a low-color-variance portrait photo (correctly renders as vivid brown, not grey, since that's genuinely the source content) and a multi-hue illustrated set (renders as a clearly layered, multi-tone mesh) — see `menubar/design/FIDELITY.md`'s color/depth test.

Also fixed: the "z" double-strike bars were spaced too tightly to read as two lines at small sizes (merged into one ragged mark below ~24pt); cards across every tab now use a distinctly brighter/more opaque `GlassCard` treatment instead of a barely-visible `Color.white.opacity(0.08)` background; the rotary dials are light/glassy (matching the approved mockups) instead of a dark puck; every wallpaper thumbnail across the popover, Sets, Weather, and Creators Publish now shares one 16:10 aspect ratio instead of three slightly different ones; the popover shows a clean "X min left" label instead of no time-remaining indicator at all.

**`shrutz weather --json` now includes `conditions`.** The full 7-value weather-condition vocabulary (`clear cloudy fog rain snow storm night`), so the menu bar app's condition picker can be driven entirely from JSON instead of hardcoding its own copy of the list.
